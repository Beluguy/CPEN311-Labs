...
module tb_rtl_task1();
	...
	initial begin
		...
		#1;
		clock;
		if(LEDR[9] == 1'b1) begin //LEDR[9] shows rdy
			$display("Correct output for LEDR9: init is in rdy state");
		end
		else begin
			$error("Incorrect output for LEDR9: %b. Expected 1", LEDR[9]);
			failed = failed + 1;
		end
		...
		for(i = 0; i < 256; i = i + 1) begin
			...
			if((dut.init.addr == i)&&(dut.init.wrdata == i)&&(dut.init.wren == 1)) begin
				...
				$error("Incorrect output for addr, wrdata, wren: %d %d %b. Expected output : %d, %d, 1", dut.init.addr, dut.init.wrdata, dut.init.wren, i, i);
			failed = failed + 1;
			end
			KEY[0] = 1'b1;
		...
		if((dut.init.addr == 255)&&(dut.init.wrdata == 255)&&(dut.init.wren == 0)) begin
			...
			$error("Incorrect output for addr, wrdata, wren: %d %d %b. Expected output : 255, 255, 0", dut.init.addr, dut.init.wrdata, dut.init.wren);
			failed = failed + 1;	
		end
		...
		if(LEDR[9] == 1'b1) $display("Correct: rdy is %d after initialized", LEDR[9]);
		else begin
			$error("rdy is %d after initialized", LEDR[9]);
			failed = failed + 1;	
		end
		...
		if(LEDR[9] == 1'b0) $display("Correct: rdy is %d after reset", LEDR[9]);
		else begin
			$error("rdy is %d after reset", LEDR[9]);
			failed = failed + 1;	
		end
